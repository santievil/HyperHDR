...
class ImageDataAML : public QSharedData
{
public:
	typedef Pixel_T pixel_type;

	ImageDataAML(int width, int height, const Pixel_T background) :
		_width(width),
		_height(height),
	...
	ImageDataAML(const ImageDataAML & other) :
		QSharedData(other),
		_width(other._width),
	...
	ImageDataAML& operator=(ImageDataAML rhs)
	{
		rhs.swap(*this);
		return *this;
	}

	void swap(ImageDataAML& s) noexcept
	{
		using std::swap;
	...
	ImageDataAML(ImageDataAML&& src) noexcept
		: _width(0)
		, _height(0)
		, _pixels(NULL)
	{
	...
	ImageDataAML& operator=(ImageDataAML&& src) noexcept
	{
		src.swap(*this);
		return *this;
	}
